...
try:
    cur_inf_fp = open(cur_inf, 'r')
    # skip the header
    cur_inf_fp.readline()
    cur_line = cur_inf_fp.readline()
    # split the line
    cur_paras = cur_line.strip().split(',')
    # get the parameters
    cur_RA = cur_paras[1]
    cur_Dec = cur_paras[2]
    cur_centeral_freq_low = float(cur_paras[3])
    cur_bandwith = float(cur_paras[4])
    # print curRA
    # print curDec
...
class SinglePulseEventGroup(object):
    ...
    def find_peak_score(self):
        ...
        if (df1 is not None) and (df2 is not None) and (df3 is not None) and (df4 is not None) and \
            ...
            model1 = sm.WLS(df1['SNR'], sm.add_constant(df1['DM']), weights=df1['SNR'] - 5)
            coef1 = model1.fit()

            model2 = sm.WLS(df2['SNR'], sm.add_constant(df2['DM']), weights=df2['SNR'] - 5)
            coef2 = model2.fit()

            model3 = sm.WLS(df3['SNR'], sm.add_constant(df3['DM']), weights=df3['SNR'] - 5)
            coef3 = model3.fit()

            model4 = sm.WLS(df4['SNR'], sm.add_constant(df4['DM']), weights=df4['SNR'] - 5)
            coef4 = model4.fit()

            model5 = sm.WLS(df5['SNR'], sm.add_constant(df5['DM']), weights=df5['SNR'] - 5)
            coef5 = model5.fit()

            model6 = sm.WLS(df6['SNR'], sm.add_constant(df6['DM']), weights=df6['SNR'] - 5)
            coef6 = model6.fit()
